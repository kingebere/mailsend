  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const campaign = (await cRes.json()) as Report['campaign']
  
        const analytics = aRes.ok
          ? (await aRes.json()) as { campaigns?: any[] }
          : {}
  
        const detail: CampaignDetail = dRes.ok
          ? await dRes.json()
          : {}
  
        const found = analytics?.campaigns?.find(
          (c: any) => c.id === id
        ) || {}
