...
import {
  Send, Save, Eye, AlertCircle, ChevronDown,
...
const MERGE_TAGS = ['{{first_name}}', '{{last_name}}', '{{email}}', '{{unsubscribe_link}}']
...
export default function NewCampaignPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([g, t, me, contacts]) => {
      setGroups(Array.isArray(g) ? g : [])
      setTemplates(Array.isArray(t) ? t : [])
      setTotalContacts(contacts?.total || 0)
      if (me?.user) {
        setForm(f => ({
          ...f,
          fromEmail: me.user.fromEmail || '',
          fromName: me.user.fromName || '',
        }))
      }
    })
  }, [])
  ...
  async function handleSave(send = false) {
    ...
    try {
      ...
      if (send || form.sendNow) {
        const sendRes = await fetch(`/api/campaigns/${data.id}/send`, { method: 'POST' })
        const sendData = await sendRes.json()
        if (!sendRes.ok) { alert(sendData.error); return }
    ...
    <div className="p-8 max-w-7xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => handleSave(false)} disabled={loading} className="btn btn-secondary">
            {saved ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <Save className="w-4 h-4" />}
            Save draft
          </button>
          <button onClick={() => handleSave(true)} disabled={loading} className="btn btn-primary">
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-2 space-y-5">
          ...
          <div className="card p-5 space-y-4">
            ...
            <div>
              <label className="label">Campaign name <span className="text-gray-400">(internal)</span></label>
            ...
            <div>
              <label className="label">Preview text <span className="text-gray-400">(shown after subject in inbox)</span></label>
              <input className="input" value={form.previewText} onChange={e => set('previewText', e.target.value)}
          ...
          <div className="card p-5 space-y-4">
            ...
            <div>
              <label className="label">From email <span className="text-gray-400">(must be SES-verified)</span></label>
              <input className="input" type="email" value={form.fromEmail} onChange={e => set('fromEmail', e.target.value)}
                placeholder="hello@yourdomain.com" />
            </div>
            <div>
              <label className="label">Reply-to <span className="text-gray-400">(optional)</span></label>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-brand-50 rounded-lg">
              ...
              <div className="text-xs text-brand-700">
                ...
                {' · '}Estimated cost: <span className="font-semibold">{estimateCost(recipientCount)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={form.sendNow} onChange={() => set('sendNow', true)} className="accent-brand-600" />
                <span className="text-sm text-gray-700">Send immediately</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={!form.sendNow} onChange={() => set('sendNow', false)} className="accent-brand-600" />
        ...
        <div className="col-span-3 space-y-4">
          <div className="card">
            ...
            {showPreview ? (
              <div className="p-4">
                <div className="text-xs text-gray-400 mb-2">Preview (merge tags shown as-is):</div>
                <div
          ...
          <div className="flex items-start gap-2.5 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
            ...
            <div>
              <strong>Important:</strong> Every email must contain <code className="bg-amber-100 px-1 rounded text-xs">{'{{unsubscribe_link}}'}</code>.
              It's already in the default template. Removing it violates CAN-SPAM law and will hurt your deliverability.
            </div>
          </div>
        </div>
      </div>
