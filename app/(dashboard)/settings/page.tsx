...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const [meRes, sesRes, apiKeysRes] = await Promise.all([
          fetch('/api/auth/me'),
          fetch('/api/ses/validate'),
          fetch('/api/api-keys'),
        ])

        const meData: MeResponse = meRes.ok ? await meRes.json() : {}
        const sesData: SESStatus = sesRes.ok ? await sesRes.json() : { connected: false }
        ...
        if (meData.user) {
          ...
          if (meData.user.awsAccessKeyId) {
            setSes((s) => ({
              ...s,
              awsAccessKeyId: meData.user?.awsAccessKeyId || '',
              awsRegion: meData.user?.awsRegion || 'us-east-1',
            }))
          }
        }

        setSesStatus(sesData)
  ...
  async function validateSES() {
    ...
    try {
      ...
      const data: ValidateSESResponse = await res.json()
      ...
      if (res.ok) {
        ...
        setSesStatus({
          connected: false,
          error: data.error || 'Failed to validate SES',
        })
      }
    } catch (error) {
      console.error('Failed to validate SES:', error)
      setSesStatus({
        connected: false,
        error: 'Failed to validate SES',
      })
    } finally {
      setValidating(false)
    }
  ...
  async function createApiKey() {
    ...
    try {
      ...
      if (res.ok && data.key) {
        ...
        const keysData: ApiKey[] = keysRes.ok ? await keysRes.json() : []
    ...
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Manage your account, AWS SES connection, and API keys
        </p>
      </div>

      <Section
        title="Profile & Sender defaults"
        desc="Set your default sender name and email for campaigns"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setProfile((p) => ({ ...p, fromEmail: e.target.value }))
                }
      ...
      <Section
        title="AWS SES Connection"
        desc="Connect your AWS account to send emails. Create an IAM user with ses:SendEmail permissions only."
      >
        {sesStatus && (
          <div
            className={`flex items-center gap-2.5 p-3 rounded-lg mb-5 text-sm ${
              sesStatus.connected
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              value={ses.awsAccessKeyId}
              onChange={(e) =>
                setSes((s) => ({ ...s, awsAccessKeyId: e.target.value }))
              }
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={ses.awsSecretAccessKey}
                onChange={(e) =>
                  setSes((s) => ({ ...s, awsSecretAccessKey: e.target.value }))
                }
          ...
          <div>
            ...
            <select
              ...
              value={ses.awsRegion}
              onChange={(e) =>
                setSes((s) => ({ ...s, awsRegion: e.target.value }))
              }
            >
              {[
                'us-east-1',
          ...
          <button
            onClick={validateSES}
            disabled={
              validating || !ses.awsAccessKeyId || !ses.awsSecretAccessKey
            }
            className="btn btn-primary"
          >
        ...
        <div className="mt-5 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-start gap-2 text-sm text-amber-800">
            ...
            <div>
              <strong>IAM minimum permissions:</strong> Create an IAM user with
              only these two permissions:
              <code className="bg-amber-100 px-1 rounded mx-1 text-xs">
                ses:SendEmail
              </code>
      ...
      <Section
        title="Bounce & Complaint Webhook"
        desc="Set this URL in your AWS SNS topic to automatically handle bounces and spam complaints."
      >
        <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-gray-500">
              Your SNS webhook URL
            </span>
            <button
        ...
        <div className="mt-3 text-sm text-gray-600 space-y-1">
          ...
          <ol className="list-decimal list-inside space-y-1 text-sm text-gray-500">
            ...
            <li>
              Create a new topic, then create an HTTPS subscription with the URL
              above
            </li>
            <li>
              In SES, set your verified identity&apos;s notification settings to
              use this SNS topic for Bounces and Complaints
            </li>
          </ol>
        </div>
        <a
      ...
      <Section
        title="API Keys"
        desc="Use API keys to send transactional emails from your app or integrate with Zapier."
      >
        {newKeyValue && (
