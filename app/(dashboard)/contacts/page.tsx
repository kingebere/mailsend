...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    subscribed: 'badge-green', unsubscribed: 'badge-gray',
    bounced: 'badge-red', complained: 'badge-yellow',
  }
...
function Avatar({ name, email }: { name: string; email: string }) {
  ...
  const colors = ['bg-brand-100 text-brand-700', 'bg-blue-100 text-blue-700', 'bg-green-100 text-green-700',
...
export default function ContactsPage() {
  ...
  async function handleDeleteSelected() {
    ...
    if (deleteAll) {
      // Fetch every matching contact ID (bypass pagination)
      const params = new URLSearchParams({
        page: '1', limit: '99999',
        ...(search && { search }),
        ...(status && { status }),
      ...
      const res = await fetch(`/api/contacts?${params}`)
      const data = await res.json()
      idsToDelete = (data.contacts || []).map((c: Contact) => c.id)
    } else {
      idsToDelete = Array.from(selected)
    }

    // Delete in parallel batches of 20 to avoid overwhelming the server
    const batchSize = 20
    for (let i = 0; i < idsToDelete.length; i += batchSize) {
      const batch = idsToDelete.slice(i, i + batchSize)
      await Promise.all(batch.map(id => fetch(`/api/contacts/${id}`, { method: 'DELETE' })))
    }
    ...
    <div className="p-8 max-w-7xl">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => fileRef.current?.click()} disabled={importing} className="btn btn-secondary">
      ...
      <div className="mb-5 p-4 bg-blue-50 border border-blue-100 rounded-xl">
        <div className="text-xs font-semibold text-blue-700 mb-1">CSV format guide</div>
        <div className="text-xs text-blue-600">
          Required column: <code className="bg-blue-100 px-1 rounded">email</code>.
          Optional: <code className="bg-blue-100 px-1 rounded">first_name</code>{' '}
          <code className="bg-blue-100 px-1 rounded">last_name</code>{' '}
          <code className="bg-blue-100 px-1 rounded">tags</code> (comma-separated inside quotes).
          Example row: <code className="bg-blue-100 px-1 rounded">jane@example.com,Jane,Smith,"vip,newsletter"</code>
        </div>
      </div>
      ...
      {someSelected && (
        <div className="mb-4 flex items-center gap-3 px-4 py-3 bg-brand-50 border border-brand-200 rounded-xl">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={() => setSelected(new Set())} className="btn btn-ghost text-xs py-1.5 px-3 text-brand-700">
              Deselect
            </button>
            <button
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">Try adjusting your filters or import a CSV</p>
          </div>
        ) : (
          <>
            <table className="w-full">
              ...
              <tbody>
                {contacts.map(c => {
                  ...
                  return (
                    <tr key={c.id} className={`table-row ${isSelected ? 'bg-brand-50/40' : ''}`}>
                      <td className="td">
                        <input
                          type="checkbox"
                          checked={isSelected}
                      ...
                      <td className="td">
                        <div className="flex flex-wrap gap-1">
                          {c.tags.map(t => (
                            <span key={t.tag.id} className="badge badge-gray">{t.tag.name}</span>
                          ))}
                        </div>
                      </td>
                      <td className="td"><StatusBadge status={c.status} /></td>
                      <td className="td text-xs text-gray-400">{formatDate(c.createdAt)}</td>
                      <td className="td">
                        <button
                          onClick={async () => {
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                  className="btn btn-secondary text-xs py-1.5 px-3 disabled:opacity-40">← Prev</button>
                <button disabled={page >= pages} onClick={() => setPage(p => p + 1)}
                  className="btn btn-secondary text-xs py-1.5 px-3 disabled:opacity-40">Next →</button>
              </div>
            </div>
          </>
        )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="card w-full max-w-md p-6">
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  This will permanently remove {deleteAll ? `all ${total.toLocaleString()} contacts` : `${deleteCount} selected contact${deleteCount !== 1 ? 's' : ''}`} from your list.
            ...
            <div className="flex gap-2">
              <button
                onClick={() => { setShowDeleteConfirm(false); setDeleteAll(false) }}
                className="btn btn-secondary flex-1"
              >
                Cancel
              </button>
              <button onClick={handleDeleteSelected} disabled={deleting} className="btn btn-danger flex-1">
                {deleting
                  ? <Loader2 className="w-4 h-4 animate-spin" />
                  : <Trash2 className="w-4 h-4" />}
                {deleting ? 'Deleting...' : `Yes, delete ${deleteCount.toLocaleString()}`}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="card w-full max-w-md p-6">
            ...
            <form onSubmit={handleAddContact} className="space-y-4">
              <div>
                ...
                <input className="input" type="email" required value={newContact.email}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="input" value={newContact.firstName}
                    onChange={e => setNewContact(n => ({ ...n, firstName: e.target.value }))}
                    placeholder="Jane" />
                </div>
                <div>
                  <label className="label">Last name</label>
                  <input className="input" value={newContact.lastName}
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="btn btn-primary flex-1">Add contact</button>
              </div>
            </form>
          </div>
        </div>
