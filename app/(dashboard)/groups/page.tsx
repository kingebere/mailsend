...
export default function GroupsPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (res.ok) { setShowAdd(false); setForm({ name: '', description: '' }); load() }
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Segment your audience for targeted campaigns</p>
        </div>
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-800">
        <strong>What are groups?</strong> Groups let you send campaigns to a specific subset of your contacts.
        For example: "Newsletter subscribers", "VIP customers", "Trial users". When importing contacts via CSV,
        you can choose which group to add them to.
      </div>
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1 mb-4">Create groups to segment your audience</p>
            <button onClick={() => setShowAdd(true)} className="btn btn-primary">
            ...
            <tbody>
              {groups.map(g => (
                <tr key={g.id} className="table-row">
                  ...
                  <td className="td">
                    <span className="font-semibold text-gray-900">{g._count.members.toLocaleString()}</span>
                  </td>
                  <td className="td text-xs text-gray-400">{formatDate(g.createdAt)}</td>
                  <td className="td">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <input className="input" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="e.g. Newsletter subscribers" autoFocus />
              </div>
              <div>
